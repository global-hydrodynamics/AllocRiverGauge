...
! ----------
 1000 continue
      ...
      !! If uparea isn't valid, only lat&lat are used
      else
            !! search domain for high-res map
            if( trim(region)=='japan05' ) nn=nn*2
            area0=-9999.
            diff=-999.
            score_all0=-999
            err_are0=-999
      ...
      if( dkm0<1.e20 .and. area0<=0 )then
        write(21,'(i12,10f12.3,2i8)') id, lat,lon,area, lat0,lon0,area0, diff, err_are0, dkm0, score_all0, kXX, kYY
        ! print '(i12,6f12.3,2i8)', id, lat,lon,area, lat0,lon0, dkm0, kXX, kYY

      elseif( score_all0<1.e20 .and. area0>0 )then
        diff=area-area0
