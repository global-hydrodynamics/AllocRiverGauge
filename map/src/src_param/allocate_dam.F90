...
! input
      ...
      integer              ::  year
      real                 ::  lat0, lon0, lat, lon, area0, area, cap_mcm, elev
...
! ===============================================
      ...
      write(21,'(a,a)') '        ID       lat       lon   area_CaMa  area_GRanD       error        diff', &
                        '      ix      iy     cap_mcm  year  damname                         rivname'

      wfile2='./dam_alloc_small.txt'
      open(22, file=wfile2, form='formatted')
      write(22,'(a,a)') '        ID       lat       lon   area_CaMa  area_GRanD       error        diff', &
                        '      ix      iy     cap_mcm  year  damname                         rivname'

      wfile3='./dam_alloc_error.txt'
      open(23, file=wfile3, form='formatted')
      write(23,'(a,a)') '        ID       lat       lon   area_CaMa  area_GRanD       error        diff', &
                        '      ix      iy     cap_mcm  year  damname                         rivname'



 1000 continue
      read(11,*,end=1090) id, lat0,lon0,area0, buf,buf,buf, damname, rivname, cap_mcm, elev, year
